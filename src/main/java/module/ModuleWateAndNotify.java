...
public class ModuleWateAndNotify {
    ...
    private boolean function2(String pathClass, String nameMethod, int index) {
        ...
        if (!foundClassImport.isEmpty()) {
            for (String foundClassImport1 : foundClassImport) {
                ...
                foundCreatedNewObject.visit(cu, splitPathToNameClass(pathClass));

                ArrayList<List<Expression>> foundCreatedNewObjectArgs = foundCreatedNewObject.getFoundCreatedNewObjectArgs();
    ...
    private <T> boolean rec2(T cu, String nameObj) {
        ...
        if (!foundNotify(cu, nameObj)) {
            ...
            byParameterValue.visit((CompilationUnit) cu, nameObj);
